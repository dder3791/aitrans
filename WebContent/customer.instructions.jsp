...
<html>
  <head>
    ...
    <title><spring:message  code='客户须知'/></title>
      ...
      <div class="quality_control">
        ...
        <div class="content_public">
          ...
          <div class="title_public">
            <h2><spring:message  code='客户须知'/></h2>
          </div>
          
          <ul class="quality_control_list" style="font-size:15px;">
            <li><b>1、<spring:message  code='客户须满足的条件说明。'/></b></li><br>
			<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='客户需要进行注册：客户注册可以根据邮箱注册，或手机号注册信息认证（可选）：'/></li><br>
			<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='客户认证分为企业认证和个人认证。'/></li><br>
 	 		<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='企业认证：项目写有关企业基本信息，上传企业营业执照等。'/></li><br>
 	 		<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='个人认证：项目写客户真实姓名，有关证件号，上传客户正反面证件照片。'/></li><br>

			<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='账户充值（发布项目时备用金不能为0元）；'/></li><br>
			<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='充值时，客户可以根据所发项目的支付金额进行充值（充值方式：联系客服，或第三方支付），最终获得需求发布的权限。'/></li><br>
			<li><b>2、<spring:message  code='客户可以享受的服务介绍。'/></b></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='客户可以进行在线的多种模式发布项目（系统推送，招标模式，点对点选定译员）。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='平台给予客户的保证：客户注册过程中会签订平台提出的有关协议，已保证客户的利益不受损失。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='平台提供的翻译价格的合理：译员报价或者系统报价都必须依靠平台的报价系统。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='客户可以在平台上任选一位或多位译员等，客户以选定译员模式发布项目时，可以选择多位译员是平台进行推送，直到有一位译员接受任务，使平台形成相应的订单，其他译员则失效。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='客户订单达到一定数量（连续3个月订单成交量在70以上）即为平台vip客户，可享受招标模式的项目在平台展示时可以排名在前。'/></li><br>
<li><b>3、<spring:message  code='客户的使用方式介绍。'/></b></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='如何发布项目：根据平台提示填写项目有关信息，选择在线报价，填写相关信息，对此进行询价作为发布项目的参考，选择快速发布，确认项目信息后即可发布。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='如何选择项目发布模式：项目发布模式分为系统分配，招标模式，选定译员模式3种'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='系统分配模式：平台会根据客户需求计算项目金额，客户可以随意改动此金额，但不能低于此金额。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='招标模式：平台会根据客户需求计算需求金额，客户可以随意改动此金额。'/></li><br>
<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='选定译员模式：平台根据客户选中的译员报价计算需求金额，客户可以随意改动，但不能低于译员报价。'/></li><br>
            <li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='如何针对未发布的需求进行修改以及重新发布：客户编写了一个未发布的需求，可以在客户中心--发布模式--未发布，即可展示该客户所有未发布的需求，指定一个未发布需求即可对其进行操作。'/></li><br>
           <li><b>4、<spring:message  code='平台为客户提供的便利说明。'/></b></li><br>
            <li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='平台提供线上客户的需求推送：客户在发布一个选定译员或者系统分配模式的需求之后，平台会在一定的时间段内对译员进行分批推送，若是招标需求，平台将会实时展示在平台的需求模块，以便译员查找。'/></li><br>
            <li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='平台提供高质量、各种语种领域的译员：平台上的译员都是经过有关翻译专家培训，点评之后并赋予一定的翻译等级才可以接受任务，进行翻译工作。'/></li><br>
            <li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<spring:message  code='平台提供配套的关于译员翻译的评价系统，以便于了解译员信息。'/></li><br>
			</ul>
        </div>
      </div>
      <!--  质量控制 内容结束 -->
