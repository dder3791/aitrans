            ...
            <div class="my_customer_right col-sm-10">
              ...
              <div class="">
                <ul id="suerTab" class="nav nav-tabs">
                 <li class="active"><a href="#personals" data-toggle="tab"><spring:message  code='翻译语言详情'/></a></li>
                </ul>
              </div>
                ...
                <div class="myTabContent tab-content" >
                   ...
                   <div class="tab-pane fade in active" id="personals"  style="padding:0px 60px 0px 40px;">
                    <h4 style="font-size: 25px;color: #0044BB"><spring:message  code='${language }'/></h4>
                      <div class="basic_data">
                		...
                		<div class="form-group" style="width:100%">
                        	...
                        	<div class="row" style="margin-top:30px;">
                        		<table border="1" style="width:100%">
                        			<tr class="thead">
                        				...
                        				<th><spring:message  code='翻译等级'/></th>
                        				<th><spring:message  code='翻译价格<br>(元/千字)'/></th>
                        				<th><spring:message  code='校对等级'/></th>
                        				<th><spring:message  code='校对价格<br>(元/千字)'/></th>
                        				<th><spring:message  code='审核等级'/></th>
                        				<th><spring:message  code='审核价格<br>(元/千字)'/></th>
                        				<th><spring:message  code='操作'/></th>
                        			</tr>
                        			<c:forEach items="${quoList }" var="quo" varStatus="q">
                        				<c:if test="${q.count%2 == 0 }">
                        					<tr style="background-color: #F0F0F0;">
	                        					<td><font style="color:#0066FF">${quo.domain }</font></td>
		                        				<td>
		                        					<c:if test="${quo.tranlevels == 0 }"><spring:message  code='暂无'/></c:if>
		                        					<c:if test="${quo.tranlevels != 0 }">${quo.tranlevels }</c:if>
		                        				</td>
		                        				<td><font color="#CC0000">${quo.tranPrice }</font></td>
		                        				<td>
		                        					<c:if test="${quo.proolevels == 0 }"><spring:message  code='暂无'/></c:if>
		                        					<c:if test="${quo.proolevels != 0 }">${quo.proolevels }</c:if>
		                        				</td>
		                        				<td><font color="#CC0000">${quo.prooPrice }</font></td>
		                        				<td>
		                        					<c:if test="${quo.auditlevels == 0 }"><spring:message  code='暂无'/></c:if>
		                        					<c:if test="${quo.auditlevels != 0 }">${quo.auditlevels }</c:if>
		                        				</td>
		                        				<td><font color="#CC0000">${quo.auditPrice }</font></td>
		                        				<td>
		                        					<a href="/translator/translatorAction_reviewUU.do?id=${quo.id }"><button class="btn btn-info"><spring:message  code='详情'/></button></a>
		                        					<a href="/translator/translatorAction_updateUU.do?id=${quo.id }"><button class="btn btn-info"><spring:message  code='修改'/></button></a>
		                        				</td>
	                        				</tr>
                        				</c:if>
                        				<c:if test="${q.count%2 == 1 }">
                        					<tr>
	                        					<td><font style="color:#0066FF">${quo.domain }</font></td>
		                        				<td>
		                        					<c:if test="${quo.tranlevels == 0 }"><spring:message  code='暂无'/></c:if>
                          ...
                          <button class="btn btn-info" name="button" onclick="location='<%=basePath%>/translator/translatorAction_findTrans.do?page=addQuo'"><spring:message  code='返回'/></button>
                           </div>
                        </div>
                    </div>
                  </div>
