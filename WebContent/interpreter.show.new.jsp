...
//            var pageCount = 1;
				            ...
				            $.post("/translator/findInterByPage.do","mm="+mm+"&phonemsg="+domains+"&namemsg="+language+"&msg="+msgs+"&secondmsg="+levels+"&msgtype="+proof,function(dd){
				              ...
				              for(var i=0;i<data.length;i++){
				          	  ...
				          	  for(var m=0;m<data.length;m++){
				          			  append_nickname[m].innerHTML=data[m].nickname;
				          			  append_transId[m].innerHTML=data[m].id;
				          			if(data[m].language =='EN'){
				        				 append_language[m].innerHTML='<spring:message  code="英语"/>';
				        			  }else if(data[m].language =='JP'){
				        				append_language[m].innerHTML='<spring:message  code="日语"/>';
				        			  }else if(data[m].language =='KOR'){
				        				append_language[m].innerHTML='<spring:message  code="韩语"/>';
				        			  }else if(data[m].language =='FR'){
				        				append_language[m].innerHTML='<spring:message  code="法语"/>';
				        			  }else if(data[m].language =='GER'){
				        				append_language[m].innerHTML='<spring:message  code="德语"/>';
				        			  }else{
				        				  append_language[m].innerHTML="<br>";
				        			  }
				          			var domainCard=data[m].domain.split(",");
								   	//alert(domainCard[0])
								    ...
								    if(data[m].majorTotals==0 && data[m].prooTotals==0 ){
				          			...
				          			else if(data[m].degree=="本科"){
								    	 append_degree[m].innerHTML="<spring:message  code='本科'/>";
								    }
				          			else if(data[m].degree=="研究生"){
								    	 append_degree[m].innerHTML="<spring:message  code='本科'/>";
								    }
				          			else if(data[m].degree=="硕士"){
								    	 append_degree[m].innerHTML="<spring:message  code='硕士'/>";
								    }
				          			else if(data[m].degree=="博士"){
								    	 append_degree[m].innerHTML="<spring:message  code='博士'/>";
								    }else{
								    	append_degree[m].innerHTML="<br>"
								    }
								    if(data[m].degree=='' ||data[m].degree==null || data[m].degree=='[]'  ){
								    	append_card_btn[m].setAttribute("style", "margin-top:54px");
								    } 
				          			append_transphoto[m].setAttribute("src",userUrl);
				          			append_checkTrans[m].setAttribute("href",checkUrl);
				          			  lookTrans[m].setAttribute("href","/translator/translatorAction_findTransById.do?transId="+data[m].id);
							 }  
    ...
    $("#page").initPage(totalCount,pageNum,GG.kk);
		...
		$("#button").click(function(){
			...
			msgs=$("#msg").val();
      	    ...
      	    //给予模糊查询字段赋值msg
			$("#msg").prop("value", msgs);
            $.post("/translator/findInterByPage.do","mm="+1+"&phonemsg="+domains+"&namemsg="+language+"&msg="+msgs+"&secondmsg="+levels+"&msgtype="+proof,function(dd){
            	//给予o进行赋值重新初始化时不执行方法
				o = 2;
				//获取page对象
			 /*  var page = data.page;
             ...
             for(var i=0;i<data.length;i++){
              ...
              var append_tranTotal=$(".append_card [id=tranTotals]");
          	  ...
          	  for(var m=0;m<data.length;m++){
          		  ...
          		  if(language==null ||language ==''){
          			...
          			append_language[m].innerHTML=language;
          		  }
      			  
          			var domainCard=data[m].domain.split(",");
          			var firstDomain="";//第一个领域
				   	//alert(domainCard[0])
				   	for(var z=0;z<domainCard.length;z++){
				   		if(domainCard[z]=='日常'){
				   			domainCard[z]="<spring:message  code='日常'/>"
				   		...
				   		if(z==0){
				   			firstDomain=domainCard[z];//给第一ge领域,赋值
				   		}
				   		
				   		if(domains==domainCard[z] && z!=0){
				   			//如果是选中的领域
				   			domainCard[0]="";//第一领域显示为首位
				   			domainCard[z]=firstDomain
				   		}else if(domains==domainCard[z] && z==0){
				   			domainCard[0]="";
				   		}
				   	}
				    ...
				    if(b!=null && b!=""){
				    	...
				    	append_domain[m].innerHTML="<span style='color: red;'>"+domains+"</span>"+b; 
				    }else{
				    	append_domain[m].innerHTML="<span style='color: red;'>"+domains+"</span>";
				    }
				    ...
				    if(data[m].level == 1){
  				  	...
  				  	if(data[m].degree=='' ||data[m].degree==null ){
          	...
          	//判断总记录数是否为0 
				...
				if(page2.totalCount==0){
					...
					//如果不为0则初始化页面
					$("#page").initPage(page2.totalCount,page2.pageNum, GG.kk); 
				} 
            },"json");
			         } 
    		 })
    ...
    $.fn.capacityFixed = function(options) {
    	    ...
    	    function findTransQuotation(id){
        	    var url="/cilentCustomer/transInfo.do";
        	    $.post(url,{"id":id,"type":"checkTrans"},function(data){
      ...
      <div class="interpreter_conts">
        ...
        <div class="interpreter_search">
          <form class="form-inline">
          		<div class="row">
               		<label  class="col-xs-2 rowlabel"><b><spring:message code='专业领域'/></b></label>
              		<div class="col-xs-2">
              			<select class="form-control select_wid" style="width:100%;" name="" id="domain">
			                <option value=""><spring:message  code="全部"/></option>
              		...
              		<label  class="col-xs-1 rowlabel"><b><spring:message code='选择语对'/></b></label>
              		<div class="col-xs-2">
              			<select class="form-control" style="width:100%;" name="language" id="language">
              				<option value=""><spring:message  code="全部"/></option>
              		...
              		<label  class="col-xs-1 rowlabel"><b><spring:message code='翻译流程'/></b></label>
              		<div class="col-xs-2">
              			<select class="form-control proof" style="width:100%;" name="proof" id="proof">
			                <option value=""><spring:message  code="全部"/></option>
              	...
              	<div class="row">
              		...
              		<label  class="col-xs-1 rowlabel"><b><spring:message code='译员昵称'/></b></label>
              		<div class="col-xs-4">
              			<input type="text" class="form-control" style="width:100%;" placeholder="<spring:message  code="请输入要查询的译员姓名"/>" id="msg">
              		</div>
              		<div class="col-xs-1">
              			<button class="btn btn-default btn-primary" style="width: 100%" type="button" id="button"><spring:message  code="搜索"/></button>
              		</div>
              	</div>
