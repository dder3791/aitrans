...
<!--标注作者-->
<title><spring:message  code='语料库'/></title>
<link rel="stylesheet"
...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/patent.knowledge.css" />
<link rel="stylesheet"
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/patent.corpus.js"></script>
			...
			"kk" : function(mm) {
				...
				if (o != 2) {//判断是否为搜索的页面初始化进入
					$
							.ajax({
								type : "POST",
								url : "/corpus/corpusAction_queryByAll.do",
								data : "pageNum=" + mm + "&language="
								...
								$("#domain").append("<option value="+domains[x].id+">"+str+ "</option>")
									...
									//遍历添加返回的术语信息
									var oul = $(".case_uls");
									for (var j = 0; j < corpuss.length; j++) {
										var corpusOrigin = "";
										var corpusTarget = "";
										corpusOrigin = corpuss[j].corpusOrigin;
										corpusTarget = corpuss[j].corpusTarget;
										if (corpuss[j].corpusOrigin.length >= 30) {
											corpusOrigin = corpuss[j].corpusOrigin
													.substring(0, 30)
													+ "...";
										} else {
										...
										if (corpuss[j].corpusTarget.length >= 30) {
											corpusTarget = corpuss[j].corpusTarget
													.substring(0, 30)
													+ "...";
										} else {
										...
										if (msg != "" && msg != null
											...
											if (corpusOrigin.indexOf(msg) >= 0) {
												var str1 = corpusOrigin
														.substring(
																0,
																corpusOrigin
																		.indexOf(msg));
												var str2 = corpusOrigin
														.substring(
																(corpusOrigin
																		.indexOf(msg) + msg.length),
																corpusOrigin.length)
												corpusOrigin = str1
														+ "<font color=red>"
														+ msg + "</font>"
										...
										oul
												.append('<li class="case_lists btncas list-group-item clearfix" style="margin-bottom:10px;text-align:center;cursor:pointer;" name = "li" onclick="queryById('
														+ corpuss[j].id
														+ ')">'
														+ '<span class="col-sm-6">'
														+ corpusOrigin
									...
									$(".case_lists:nth-child(odd)").css(
											"background", "#f2f2f2")
									$(".btncas").hover(function() {
									...
									.ajax({
										...
										success : function(data) {
											...
											if(terms.length>0){
												...
												$("#term").hide();
												layer.msg('<spring:message  code="尚未查询到术语信息！"/>', {
													icon : 5
												});
											}
											for (var y = 0; y < terms.length; y++) {
												$("#term").after('<li class="case_lists btncas list-group-item clearfix" style="margin-bottom:10px;text-align:center;" name = "term" onclick="return false;"><span class="col-sm-6"><font color=red>'+ terms[y].termOrigin
														+ '</font></span>'															
														+ '<span class="col-sm-6"><font color=red>'
														+ terms[y].termTarget
														+ '</font></span>'
														+ '</li>');
											}
											...
											for (var j = 0; j < corpuss.length; j++) {
												/* var corpusOrigin = corpuss[j].corpusOrigin.substring(0, 20)+"...";
												var corpusTarget = corpuss[j].corpusTarget.substring(0, 20)+"..."; */
												var corpusOrigin = "";
												var corpusTarget = "";
												if (corpuss[j].corpusOrigin.length >= 30) {
													corpusOrigin = corpuss[j].corpusOrigin
												...
												/* 	if(msg!="" && msg!=null && msg!=undefined){
														...
														newMsg= page.msg
														alert(newMsg);
														newMsg=newMsg.substring(1, newMsg.length-1);
														...
														if(newMsg.indexOf("%")>0){
															...
															ms = msg.substring(i,i+1);
															if(corpusOrigin.indexOf(ms)>=0){
															
															if(i=0){
																	str1 = corpusOrigin.substring(0, m);
																	corpusOrigin=str1;
															}else{
																	
																	str2 =corpusOrigin.substring(m,corpusOrigin.indexOf(ms));
																	corpusOrigin+="<font color=red>"+ms+"</font>"+str2;
															}
															
															}
															m=corpusOrigin.indexOf(ms);
															}
														} */
												if (msg != "" && msg != null
														&& msg != undefined) {
												...
												oul
														.append('<li class="case_lists btncas list-group-item clearfix" style="margin-bottom:10px;text-align:center;cursor:pointer;" name = "li" onclick="queryById('
																...
																+ '</span>'
																+ '</li>')
												/* 	var td1 = "<td style='padding: 15px 10px;'  >"+terms[j].termOrigin+"</td>"
											...
											if (page.totalCount == 0) {
												...
												$("#page").initPage(1,
														page.pageNum, GG.kk);
												//弹出框提示没有信息
												layer.msg('<spring:message  code="尚未查询到语料信息！"/>', {
													icon : 5
												});
											} else {
												//如果不为0则造常规初始化页面
      ...
      <!--  头标签结束 -->
		...
		<div class="patent_knowledge">
			...
			<div class="checks_langu row">
				...
				<div class="col-sm-10">
					<label class="checkbox-inline"> <input type="radio"
						name="language" value="中英" checked="checked"> <spring:message  code='汉译英'/>
					</label> <label class="checkbox-inline"> <input type="radio"
						name="language" value="英中"> <spring:message  code='英译汉'/>
					</label> <label class="checkbox-inline"> <input type="radio"
			...
			<div class="patent_lists">
				<ul class="case_uls list-group">
					<li class="case_lists list-group-item clearfix"
						style="margin-bottom: 10px; text-align: center; font-size: 16px; font-weight: 700;;"
						id="term"><span class="col-sm-6"><spring:message  code='（术语）原文'/></span> <span
						class="col-sm-6"><spring:message  code='（术语）译文'/></span></li>
					  <li class="case_lists list-group-item clearfix" style="margin-bottom:10px;text-align:center;font-size:16px;font-weight:700;">
              <span class="col-sm-6"><spring:message  code='（语料）原文'/></span>
              <span class="col-sm-6"><spring:message  code='（语料）译文'/></span>
            </li>
				</ul>
			</div>
