...
<html>
  <head>
    ...
    function isConfirm(languages,id){
    	layer.confirm("您确认要删除 <font color='blue'>"+languages+"</font> 么？", {
    	    btn: ['删除','取消'], //按钮
    	    shade: false //不显示遮罩
    	}, function(){
    		window.location.href = "/translator/deleteQuos.do?languages="+languages+"&id="+id;
    	}, function(index){
    		layer.close(index);
    	})
    }
            ...
            <div class="my_customer_right col-sm-10">
              ...
              <div class="" >
                <ul id="suerTab" class="nav nav-tabs">
                  <li class="active"><a href="#basicdata" data-toggle="tab"><spring:message  code='个人资料'></spring:message></a></li>
                 <li><a id="trans" href="#realname" data-toggle="tab" ><spring:message  code='翻译信息'/></a></li>
                  <li><a href="#bankInfo" data-toggle="tab" ><spring:message  code='银行信息'/></a></li>
                 <c:if test="${translators.isVerifty == 0 }">
                  <li><a href="#personname" data-toggle="tab" onclick="isVerifty()"><spring:message  code='个人认证'/></a></li>
                  </c:if>
                </ul>
              </div>
                <!--  tab面板对应内容   -->
                <div class="myTabContent tab-content">
                	<a href="javascript:;" onmouseover="overShowTime()" onmouseout="outHideTime()" style="margin-left: 780px"><font color="red"><b><spring:message  code='查看帮助'/>?</b></font></a>
               		...
               		<div class="tab-pane fade in active" id="basicdata" style="background-color: #fff;height:450px;;width:100%;margin-top:20px;">
               			<div style="height:100%;width:80%;float:left;">
               				...
               				<div class="row">
               					<label class="col-xs-3 rowlabel-right"><spring:message  code='昵称：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.nickname }</label>
               					<label class="col-xs-2 rowlabel-right"><spring:message  code='职业：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.job }</label>
               				</div>
               				<div class="row">
               					<label class="col-xs-3 rowlabel-right"><spring:message  code='手机号：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.tel }</label>
               					<label class="col-xs-2 rowlabel-right"><spring:message  code='电子邮箱：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.email }</label>
               				</div>
               				<div class="row">
               					<label class="col-xs-3 rowlabel-right"><spring:message  code='现居地址：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.address }</label>
               					<label class="col-xs-2 rowlabel-right"><spring:message  code='工作地址：'/></label>
               					<label class="col-xs-3 rowlabel-left">${translators.companyAddress }</label>
               				</div>
               				<div class="row">
               					<label class="col-xs-3 rowlabel-right"><spring:message  code='软件使用：'/></label>
               					<label class="col-xs-3 rowlabel-left">
               					...
               					<div class="col-xs-3"><button class="btn btn-info" onclick=" location='<%=basePath%>/translator/updateBasicInfo.do'"><spring:message  code='修改基本资料'/></button></div>
               				</div>
               				<div class="row" style="margin-top:30px;">
               					<label class="col-xs-3 rowlabel-left" style="font-size: 17px;color: #0000AA;"><spring:message  code='实名认证'/></label>
               				</div>
               				<c:if test="${translators.certificationStatus == 2 }">
	               				<div class="row">
	               					<label class="col-xs-3 rowlabel-right" style="color:#00AA00;font-size:16px;"><b><spring:message  code='已通过实名认证'/></b></label>
	               				</div>
	               				<div class="row">
	               					<label class="col-xs-3 rowlabel-right"><spring:message  code='身份证号：'/></label>
	               					<label class="col-xs-3 rowlabel-left">${translators.nameid }</label>
	               					<label class="col-xs-2 rowlabel-right"><spring:message  code='出生年月：'/></label>
	               					<label class="col-xs-3 rowlabel-left">${translators.birthday }</label>
	               				</div>
	               				<div class="row">
	               					<label class="col-xs-3 rowlabel-right"><spring:message  code='真实姓名：'/></label>
	               					<label class="col-xs-3 rowlabel-left">${translators.realName }</label>
	               					<label class="col-xs-2 rowlabel-right"><spring:message  code='性别：'/></label>
	               					<label class="col-xs-3 rowlabel-left">
               				...
               				<c:if test="${translators.certificationStatus == 0 or translators.certificationStatus == null}">
               					<div class="row">
               						<div class="col-xs-12" style="text-align: center;">
               							<button class="btn btn-info" onclick="location='<%=basePath%>/translator/newIdentity.do'"><spring:message  code='前往实名认证'/></button>
               						</div>
	               				</div>
               				</c:if>
               				<c:if test="${translators.certificationStatus == 1 }">
               					<div class="row">
               						<label class="col-xs-3 rowlabel-right" style="color:#00AA55;font-size:16px;"><b><spring:message  code='实名认证中...'/></b></label>
	               				</div>
               				</c:if>
               				<c:if test="${translators.certificationStatus == 3 }">
               					<div class="row">
               						<label class="col-xs-4 rowlabel-right" style="color:#CC0000;font-size:16px;"><b><spring:message  code='实名认证未通过'/></b></label>
               						<div class="col-xs-4" style="text-align: center;">
               							<button class="btn btn-info" onclick="location='<%=basePath%>/translator/newIdentity.do'"><spring:message  code='重新认证'/></button>
               						</div>
	               				</div>
               				</c:if>
               			</div>
                  ...
                  <!-- 翻译信息 开始 -->
                  	<div class="tab-pane fade" id="realname" style="padding:20px 20px 20px 20px;">
                  	
                  	
                  		<c:forEach items="${translatorQ}" var="translatorQ">
                  			<div class="row rowborder" style="margin-top:20px;padding: 10px 0 10px 0;">
                  				<label class="col-xs-8 rowlabel-left" style="font-size: 20px;color: black;margin-left:30px;">
                  					<spring:message  code='${translatorQ.languages }'/>
                  					<c:if test="${translatorQ.applyStateT==0} "><spring:message  code='翻译审核中'/></c:if>
                  					<c:if test="${translatorQ.applyStateT==2} "><spring:message  code='翻译未通过'/></c:if>
                  					<c:if test="${translatorQ.applyStateT==1} ">${translatorQ.tranlevels }<spring:message  code='级翻译'/></c:if>
                  					<c:if test="${translatorQ.applyStateP==0} ">&nbsp;-&nbsp;<spring:message  code='校对审核中'/></c:if>
                  					<c:if test="${translatorQ.applyStateP==2} ">&nbsp;-&nbsp;<spring:message  code='校对未通过'/></c:if>
                  					<c:if test="${translators.applyStateP==1}">&nbsp;-&nbsp;${translatorQ.proolevels }<spring:message  code='级校对'/></c:if>
                  					<c:if test="${translatorQ.tranlevels!=null && translatorQ.tranlevels!=0}">&nbsp;-&nbsp;${translatorQ.tranlevels }<spring:message  code='级翻译'/></c:if>
                  					<c:if test="${translatorQ.proolevels!=null && translatorQ.proolevels!=0}">&nbsp;-&nbsp;${translatorQ.proolevels }<spring:message  code='级校对'/></c:if>
                  					<c:if test="${translatorQ.auditlevels!=null && translatorQ.auditlevels!=0}">&nbsp;-&nbsp;${translatorQ.auditlevels }<spring:message  code='级审核'/></c:if>
                  				</label>
                  				<div class="col-xs-3" style="text-align: center;float:right;">
                  					<%-- <a href="/translator/reviewLanguage.do?languages=${translatorQ.languages}&id=${translators.id}"><button class="btn btn-info">详情</button></a> --%>
                  					<a href="/translator/toupdateLanguage.do?languages=${translatorQ.languages}&id=${translators.id}"><button class="btn btn-info"><spring:message  code='修改'/></button></a>
                  					<%-- <a href="/translator/deleteQuos.do?languages=${translatorQ.languages}&id=${translators.id}"> --%><button style="background-color: #AA0000;border-color:#AA0000;" onclick="isConfirm('${translatorQ.languages}','${translators.id}');" class="btn btn-info"><spring:message  code='删除'/></button><!-- </a> -->
                  				</div>
                  				<br>
                  				<label class="col-xs-12 rowlabel-left" style="color: black;margin-left:30px;line-height: 15px;"><spring:message  code='领域：'/>${translatorQ.domain }</label>
                  			</div>
                  		</c:forEach>
                  		
                  		<!-- 译员卡片新设计 -->
                  		<%-- <c:forEach items="${translatorQ}" var="translatorQ">
								<div class="test" style="width:220px;height:290px;float:left;margin-left: 20px;margin-top: 20px;">
									<div style="width:50px;height:50px;float:left;">
										<img class="testimg1" src="/img/tuxiang.jpg" width="40" height="40">
									</div>
									<div style="width:157px;height:40px;float:left;line-height: 40px;margin-left: 10px;font-size: 20px;margin-top: 10px;color:#000088">
										${transid.nickname }
									</div>
									<div style="width:100%;margin-top: 10px;text-align: left;padding-left: 10px;float:left;">
										<font color="#00AA00" size="3px">汉英&nbsp;&nbsp;-&nbsp;&nbsp;日常</font>
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 10px;text-align: left;padding-left: 10px;float:left;">
										翻译：<font color="#CC6600" size="4px">★★★★★</font>（9999万字）
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 2px;text-align: left;padding-left: 10px;float:left;">
										<font color="#CC0000">￥&nbsp;225</font>&nbsp;&nbsp;(元/千字)
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 5px;text-align: left;padding-left: 10px;float:left;">
										校对：<font color="#FFBB00" size="4px">★★★★</font>（300万字）
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 2px;text-align: left;padding-left: 10px;float:left;">
										<font color="#CC0000">￥&nbsp;200</font>&nbsp;&nbsp;(元/千字)
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 5px;text-align: left;padding-left: 10px;float:left;">
										审核：<font color="#00AA00" size="4px">★★★</font>（100万字）
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 2px;text-align: left;padding-left: 10px;float:left;">
										<font color="#CC0000">￥&nbsp;215</font>&nbsp;&nbsp;(元/千字)
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 10px;text-align: left;text-align: center;float:left;">
										<button class="btn btn-wt">选TA</button><button class="btn btn-wt" style="margin-left: 60px;">详情</button>
									</div>
								</div>
							</c:forEach>
							...
							<c:forEach items="${translatorQ}" var="translatorQ">
								<div class="test" style="width:220px;height:290px;float:left;margin-left: 20px;margin-top: 20px;">
									<div style="width:60px;height:70px;margin-left:75px;">
										<img class="testimg" src="/img/tuxiang.jpg" width="50" height="50">
									</div>
									<div style="width:100%;height:25px;line-height: 20px;font-size: 20px;margin-top: 5px;color:#000088;text-align: center;">
										${transid.nickname }
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 10px;text-align: center;">
										<font color="#CC6600">本科</font>
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 10px;text-align: center;">
										<font color="#00AA00">汉英，德汉，日汉</font>
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 10px;text-align: center;">
										<font color="#0044BB">日常，机械，电子，电器，自动化，计算机，...</font>
									</div>
									<div style="width:100%;line-height: 20px;margin-top: 20px;text-align: center;">
										<button class="btn btn-wt">选TA</button><button class="btn btn-wt" style="margin-left: 60px;">详情</button>
									</div>
								</div>                   			
                  		</c:forEach> --%>
                   ...
                   <!--  <hr> -->
                       	...
                       	<div class="basic_data_inp" style="text-align:center;padding-top:60px;">
                       		<a href="<%=basePath%>/translator/newLanguage.do" id="addLanguage"><button type="button" class="btn btn-info"  name="button"><spring:message  code='申请新的翻译资格'/></button></a>
